...
const SKILL_RULES = [
  ...
  ["Code & engineering", /(code-review|debug|refactor|test-writer|algorithm|complexity|walkthrough|system-design|pair-program|regex|sql-coach|git-coach|dockerfile|code-optimizer|functional-programming|oop-design|design-patterns|memory-management|type-system|error-handling|remote-code-runner|code-runner|competitive-programming|contest-prep|dsa-patterns|dynamic-programming|graph-algorithms)/],
  ["DevOps & Cloud", /(ci-pipeline|kubernetes-manifest|terraform-module|slo-designer|observability-plan|aws-well-architected|azure-landing-zone|gcp-project|cloud-cost|serverless-designer|cloud-migration|git-workflow|alerting-strategy|logging-strategy|distributed-tracing|capacity-planning|postmortem-facilitator|oncall-runbook|gitops)/],
