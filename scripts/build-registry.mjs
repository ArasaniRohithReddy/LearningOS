...
const SKILL_RULES = [
  ...
  ["Plan & track", /(roadmap|schedul|spaced-repetition|project-mentor|progress|onboarding|career-ladder|learner-memory|learning-profile|session-resume)/],
  ...
  ["Security", /(secure-code|owasp|security-hardening|incident-postmortem|dependency-audit|auth-designer|incident-response|jwt-security|secrets-management|csp-headers|sql-injection|api-security|cryptography)/],
  ...
  ["Code & engineering", /(code-review|debug|refactor|test-writer|algorithm|complexity|walkthrough|system-design|pair-program|regex|sql-coach|git-coach|dockerfile|code-optimizer|functional-programming|oop-design|design-patterns|memory-management|type-system|error-handling|remote-code-runner|code-runner)/],
  ["DevOps & Cloud", /(ci-pipeline|kubernetes-manifest|terraform-module|slo-designer|observability-plan|aws-well-architected|azure-landing-zone|gcp-project|cloud-cost|serverless-designer|cloud-migration|git-workflow|alerting-strategy|logging-strategy|distributed-tracing|capacity-planning|postmortem-facilitator|oncall-runbook)/],
  ...
  ["Backend & systems", /(caching-strategy|rate-limiter|message-queue|database-index|concurrency-coach|microservices|webhook|idempotency|saga-pattern|event-sourcing|api-pagination|graphql-schema|grpc)/],
  ["Architecture & design", /(architecture|api-design|data-modeling|threat-model|tech-comparison|estimation|openapi)/],
  ...
  ["AI & data", /(prompt|rag-designer|eval-designer|dataset|fine-tuning|model-selection|feature-engineering|ml-pipeline|agent-designer|embeddings|model-monitoring|ml-experiment|llm-guardrails|vector-db|data-labeling|ab-test|context-window|multi-agent|llm-cost|structured-output|hallucination)/],
