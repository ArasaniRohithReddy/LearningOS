...
const SKILL_RULES = [
  ...
  ["Teaching & curriculum", /(curriculum-designer|lesson-plan|reading-list|peer-review|hackathon|exam-strategy|cognitive-load|confidence-calibration|interleaving|item-writing|learning-objective|mistake-log|productive-failure|retrieval-practice|self-explanation)/],
  ...
  ["Security", /(secure-code|owasp|security-hardening|incident-postmortem|dependency-audit|auth-designer|incident-response|jwt-security|secrets-management|csp-headers|sql-injection|api-security|cryptography|supply-chain-security|broken-access-control|detection-engineering|phishing-resistant|cloud-iam-least-privilege|vulnerability-prioritization|oauth2-oidc-security|privacy-by-design|ai-agent-permissions|browser-security-model|compliance-control-mapping|dfir-evidence-triage|post-quantum-crypto|ransomware-resilience|secure-sdlc|security-logging-audit|threat-hunting|zero-trust)/],
  ["Practice & assess", /(practice|quiz|flashcard|mock-exam|assessment|gap-analysis|rubric|exam-blueprint|exam-drill)/],
  ["Databases & storage", /(database-selection|sql-query-explainer|database-migration|nosql-data-modeling|transaction-isolation|sharding|storage-engine|mvcc-vacuum|replication-topology)/],
  ["Networking & OS", /(networking-fundamentals|tls-ssl|load-balancing|dns-coach|linux-command|shell-scripting|os-internals)/],
  ["Code & engineering", /(code-review|debug|refactor|test-writer|algorithm|complexity|walkthrough|system-design|pair-program|regex|sql-coach|git-coach|dockerfile|code-optimizer|functional-programming|oop-design|design-patterns|memory-management|type-system|error-handling|remote-code-runner|code-runner|competitive-programming|contest-prep|dsa-patterns|dynamic-programming|graph-algorithms|bit-manipulation|recursion-backtracking|math-for-programming|sorting-searching|hash-table-internals|floating-point|cpu-cache-performance|memory-model-lockfree|probabilistic-data-structures)/],
  ["DevOps & Cloud", /(ci-pipeline|kubernetes-manifest|terraform-module|slo-designer|observability-plan|aws-well-architected|azure-landing-zone|gcp-project|cloud-cost|serverless-designer|cloud-migration|git-workflow|alerting-strategy|logging-strategy|distributed-tracing|capacity-planning|postmortem-facilitator|oncall-runbook|gitops|feature-flags|disaster-recovery|cloud-private-connectivity|gcp-well-architected|dora-metrics)/],
  ["Data & analytics", /(data-pipeline|streaming-pipeline|dbt-model|dashboard-designer|data-quality|spark-job|power-bi|dax|excel-formula|data-warehouse|cdc-pipeline|data-catalog|airflow-dag|data-contract|lakehouse|snowflake-performance|unity-catalog|backfill-and-reprocessing|bigquery-optimization|data-observability|data-vault|dbt-semantic-layer|parquet-internals)/],
  ...
  ["Backend & systems", /(caching-strategy|rate-limiter|message-queue|database-index|concurrency-coach|microservices|webhook|idempotency|saga-pattern|event-sourcing|api-pagination|graphql-schema|grpc|cqrs|circuit-breaker|retry-backoff|api-versioning|connection-pooling|consensus-explainer|schema-evolution|stream-windowing)/],
  ["Architecture & design", /(architecture|api-design|data-modeling|threat-model|tech-comparison|estimation|openapi|consistency-models|domain-driven-design)/],
  ["Research & news", /(research|digest|feed-curator|docs-finder|blog-finder|repo-finder|paper|literature)/],
  ["Communication & soft skills", /(conflict-resolution|feedback-giver|negotiation-coach|email-writing|public-speaking|time-management|exec-communication)/],
  ["Writing & docs", /(writing|readme|adr-writer|runbook|documentation|changelog)/],
  ["Career & communication", /(resume|star-story|portfolio|slide|demo-script|case-study|cover-letter|linkedin|salary|whiteboard|coding-interview|data-interview|frontend-interview|pm-interview|sre-interview|offer-evaluation|customer-interview|em-interview|interview-debrief|leadership-principles|job-search|promotion-packet|take-home-assignment)/],
  ["Product & process", /(user-story|prd-writer|okr-coach|retrospective|standup|one-on-one|competitive-analysis|pricing-strategy|feature-prioritization|metrics-definition|stakeholder-management|business-case|engineering-metrics|kanban-flow|product-discovery|roadmap-builder)/],
  ...
  ["AI & data", /(prompt|rag-designer|rag-evaluation|eval-designer|dataset|fine-tuning|model-selection|feature-engineering|ml-pipeline|agent-designer|embeddings|model-monitoring|ml-experiment|llm-guardrails|vector-db|data-labeling|ab-test|context-window|multi-agent|llm-cost|structured-output|hallucination|mcp-server|function-calling|agent-evaluation|agent-memory|hybrid-search|reranking|ai-governance|preference-tuning|reasoning-models|model-explainability)/],
  ["Enterprise platforms", /(abap-rap|power-fx|apex-soql|lwc-component|salesforce|servicenow|glide-scripting|dataverse|dynamics-365|sap-|workday|mulesoft|power-platform|power-automate|power-apps)/],
  ["Visualization & diagramming", /(sequence-diagram|state-machine-visualizer|er-diagram|concept-map|latex-math|dual-coding|diagram-accessibility|data-viz|diagram-as-code|diagram-review)/],
