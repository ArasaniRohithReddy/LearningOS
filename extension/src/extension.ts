...
async function setupWorkspace(): Promise<void> {
  ...
  const pick = await vscode.window.showInformationMessage(
    "Make all 128 LearningOS specialist agents + 519 skills available in Chat (agent picker + /skills)?",
...
async function offerFirstRunSetup(context: vscode.ExtensionContext): Promise<void> {
  ...
  const choice = await vscode.window.showInformationMessage(
    "Set up LearningOS — deploy 128 specialist agents + 519 skills to ~/.copilot " +
...
async function setupEverything(context: vscode.ExtensionContext, opts: { consented?: boolean } = {}): Promise<void> {
  if (!opts.consented) {
    const go = await vscode.window.showInformationMessage(
      "Set up LearningOS: deploy the full catalog (128 specialist agents + 519 skills) to your user " +
