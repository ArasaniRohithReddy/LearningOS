...
- **99 new skills across 11 domains** — pedagogy/visual-learning (self-explanation, retrieval-practice, item-writing, interleaving, worked→faded examples, dual-coding, elaborative-interrogation, cognitive-load, concept-map generation); backend/arch/DB (API versioning, connection pooling, consensus, hexagonal architecture, schema evolution, stream windowing, Flink); DevOps (Ansible, container-runtime security, DORA metrics, GitHub Actions OIDC, K8s scheduling/storage, Kustomize); data (backfill/reprocessing, BigQuery optimization, data observability, Data Vault, dbt semantic layer, Parquet internals); career/people (customer & EM interviews, engineering metrics, interview debrief, job-search strategy, Kanban flow, leadership-principles drill, product discovery, promotion packet, roadmap builder, take-home coach); web/mobile (Angular signals, browser storage, INP optimization, modern CSS, Svelte runes, View Transitions, Web Components); AI/ML (AI governance, document parsing, gradient boosting, LLM observability, model explainability, preference tuning, reasoning models); cloud + certs (CloudFormation, CloudFront, KMS envelope encryption, Organizations SCP, AWS SAA / AZ-104 / GCP ACE drills, App Service, hub-spoke VNet, private connectivity, GCP VPC + Well-Architected, OCI core); emerging/enterprise (OpenXR, Zephyr RTOS, Slurm HPC, MPI/OpenMP, PostGIS, ROS 2, Nextflow, DataWeave, quantum error mitigation, WebGPU, Dataverse, ZK proofs); languages/CS (C++ move semantics + STL, CPU-cache performance, Go context, Haskell typeclasses/monads, lock-free memory model, probabilistic data structures, Python pattern matching, Rust smart pointers); and defensive security (browser security model, compliance control mapping, DFIR triage, email-auth/phishing defense, Kubernetes hardening, post-quantum migration, ransomware resilience, secure-SDLC maturity, security logging/audit, threat hunting, zero-trust architecture).
- **336 hands-on labs** (up from 294) — 42 new local/free labs across cloud, K8s, data streaming, web platform, ML, HPC/robotics, and languages; every lab runs offline or via the no-install runner.
- Grounded in primary sources — RFC 8594/9110 & DMARCbis (RFC 9989-9991), Raft §5.4, kubernetes.io, DORA/SPACE/DevEx, Apache Parquet spec, BigQuery docs, NIST FIPS 203-205 & SP 800-61r3, CISA ZTMM v2.0, OWASP 2025 — with volatile facts (exam weights, pricing, LTS versions) flagged "verify on the current page" rather than asserted; all security content is blue-team only.
